...
import useEEECourses from "../../Hooks/useEEECourses";
...
const ContributeEEE = () => {
  ...
  const onSubmit = async(data) => {
    ...
    let imageCover ='';
      ...
      const result = await axiosPublic.patch(`/coursesEee/${courseCode}`,{contentType, resource})
       console.log(result)
      ...
      if(result.data.result.modifiedCount>0){
        ...
        //resource add to eee database done 
  ...
  <Helmet>
  <title>Note Nest - Contribute EEE</title>
  </Helmet>
...
<div className="mx-2 bg-cover  rounded-lg" style={{ backgroundImage: `url(${pattern})` }}>
 <div className=" mt-3 md:mt-10 border-2 border-slate-200 rounded-lg bg-[#F8F0FB]/95">
...
<div className="my-6 flex flex-col md:flex-row gap-4">
  <label className="block w-full md:w-1/2 text-sm font-semibold text-gray-700 mb-2">
    ...
    <select {...register("courseCode", { required: true })} defaultChecked={false} className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-4 font-semibold text-gray-500 shadow-sm focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500 sm:text-sm">
      {eee?.map(item => <option key={item.id} value={item.courseCode}>{item.courseCode}</option>)}
    </select>
